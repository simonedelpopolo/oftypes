...
import { buffer_, null_, promise_, undefined_ } from '../../index.js'
/**
 * @type {symbol}
 */
...
export const function_ = Object.defineProperty( oftypes, function_Symbol, {
    ...
    value: async function function_( variable, resolvers = { true: true, false: false }, payback = false ) {
        ...
        if ( await promise_( variable ) === true ) return ( oftypes__.fulfilled( payback, resolvers.false, variable ) )
    
        if ( await buffer_( variable ) === true ) return ( oftypes__.fulfilled( payback, resolvers.false, variable ) )
