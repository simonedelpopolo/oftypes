...
export function types( resolvers, payback, strict ) {
    ...
    if( typeof resolvers !== 'undefined' ){
        ...
        /**
         * - deep checking for the properties named correctly to true & false.
         */

        const resolversKeys = Object.keys( resolvers )

        if( resolversKeys.length > 2 )
            return `argument resolvers must have no more than two properties. Given resolvers: { ${ given_resolver( resolvers, resolversKeys ) } }`

        if( !( resolversKeys.includes( 'true' ) ) || !( resolversKeys.includes( 'false' ) ) )
            return `argument resolvers must have true & false has properties key names. Given resolvers: { ${ given_resolver( resolvers, resolversKeys ) } }`
 ...
 * @returns {string}
 */
function given_resolver( resolvers, resolversKeys ){
    let resolves_given_ = ''
    for ( const key in resolversKeys ) {
        let type_of = typeof resolvers[ resolversKeys[ key ] ]

        resolves_given_ += `${ resolversKeys[ key ] } : ${ type_of },`
    }
