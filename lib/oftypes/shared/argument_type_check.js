    ...
    value: async function* argument_type_check( payback, strict, resolvers, string, resolvers_argument ){
        ...
        if( resolvers_argument !== undefined ){
            ...
            if( resolvers_argument.truthy === undefined || resolvers_argument.falsy === undefined )
                yield '♠ both arguments \'truthy\' & \'falsy\' are required'
            else yield true

        }else{
            yield payback.constructor.name === 'Boolean'
                ? true
                : new `♠ only boolean is an accepted argument for payback. Given type: ${ payback.constructor.name }`.underline()

            yield strict.constructor.name === 'Boolean'
                ? true
                : `♠ only boolean is an accepted argument for strict. Given type: ${ strict.constructor.name }`.underline()

            yield resolvers === null
                ? `♠ only Object is an accepted argument for resolvers. Given type: ${ null }`.underline()
                : true

            if( resolvers !== undefined ) {
                yield resolvers.constructor.name === 'Object'
                    ? true
                    : `♠ only Object is an accepted argument for resolvers. Given type: ${ resolvers.constructor.name }`.underline()

                const resolversKeys = Object.keys( resolvers )
                resolversKeys.includes( 'true' ) && resolversKeys.includes( 'false' )
                    ? yield true
                    : yield `♠ resolvers argument must have only properties "true" & "false". Given properties: ${ resolversKeys }`.underline()
            }

            if( string !== undefined ) {
                yield string.constructor.name === 'Boolean'
                    ? true
                    : `♠ only boolean is an accepted argument for string. Given type: ${ string.constructor.name }`.underline()
            }
        }
    }
} )
